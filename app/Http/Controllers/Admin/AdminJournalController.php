...
class AdminJournalController extends Controller
{
    ...
    {
        $journal->load('author:id,name');

        return Inertia::render('admin/journal/Show', [
            'post' => array_merge($journal->toArray(), [
                'cover_image' => $journal->cover_image ? asset('storage/'.$journal->cover_image) : null,
                'tags_array' => $journal->tags_array,
