...
<template>
    ...
    <div class="min-h-screen bg-background text-copy">
        ...
        <aside :class="{'w-64': true, 'fixed top-0 left-0 h-full hidden lg:block': true, 'p-4': true, 'border-r-2 border-copy': true}" style="background-color: var(--secondary);">
            <div class="sticky top-4">
                <div class="relative rounded-lg -m-0.5 p-4 bg-foreground border-2 border-copy">
                    <h1 class="text-2xl font-black text-primary-content mb-6 border-b-2 border-copy pb-2" style="color: var(--primary);">
                        C.o.Y. Admin
                    </h1>
        ...
        <div class="lg:pl-64">
            ...
            <header class="sticky top-0 z-40 p-4 border-b-2 border-copy bg-background shadow-md">
                <div class="relative rounded-lg -m-0.5 p-3 bg-foreground border-2 border-copy flex justify-between items-center">
                    <button @click="sidebarOpen = true" class="lg:hidden text-copy hover:text-primary transition">
                        <svg class="size-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
                    </button>
                    <h2 class="text-lg font-bold text-copy hidden sm:block">{{ getTitle.replace('Admin | ', '') }}</h2>

                    <div class="flex items-center gap-3">
                        <span class="text-sm font-medium text-copy-light hidden sm:inline-block">Welcome, {{ user?.name }}</span>
                        <Link :href="route('logout')" method="post" as="button" class="relative rounded-lg -m-0.5 px-3 py-1 text-sm font-medium text-error-content transition border-2 border-copy bg-error-light hover:bg-error-dark">
                            Logout
                        </Link>
                    </div>
                </div>
        ...
        <Transition name="slide-fade">
            <div v-if="sidebarOpen" @click.self="sidebarOpen = false" class="lg:hidden fixed inset-0 z-50 bg-black bg-opacity-70 transition-opacity">
                <div class="fixed inset-y-0 left-0 w-64 h-full p-4 border-r-2 border-copy shadow-2xl overflow-y-auto" style="background-color: var(--secondary);">
                    <div class="relative rounded-lg -m-0.5 p-4 bg-foreground border-2 border-copy">
                        <div class="flex justify-between items-center mb-6 border-b-2 border-copy pb-2">
                            ...
                            <button @click="sidebarOpen = false" class="text-copy-light hover:text-copy transition" aria-label="Close menu">
