...
const handleFavourite = async (product: ProductCardData) => {
    ...
    try {
        ...
        if (idx === -1) {
            wishlistedIds.value.splice(wishlistedIds.value.indexOf(product.id), 1);
        } else {
...
<template>
        ...
        <div class="mx-auto max-w-screen-2xl p-4 md:p-8 lg:p-12">
            ...
            <header class="mb-8 lg:mb-10 text-copy">
                ...
                <div class="mt-4 max-w-lg rounded-lg relative">
                    <input type="text" v-model="form.search" placeholder="Search by product name or MPN..."
                    ...
                    <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                        <svg class="size-5 text-copy-lighter" fill="none" viewBox="0 0 24 24" stroke="currentColor"
            ...
            <div class="grid grid-cols-1 gap-8 lg:grid-cols-[280px_1fr]">
                ...
                <aside class="hidden lg:block">
                    <div class="sticky top-4 rounded-lg border-2 border-copy"
                        ...
                        <div class="relative rounded-lg -m-0.5 p-6 bg-foreground border-2 border-copy">
                            ...
                            <div class="mb-6">
                                ...
                                <Label for="FilterInStock"
                                    class="flex items-center space-x-3 text-copy-light cursor-pointer">
                                    ...
                                    <ul class="space-y-3">
                                        <li v-for="category in categories" :key="category.id">
                                            <Label :for="'FilterCategory-' + category.id"
                                                class="flex items-center space-x-3 text-copy-light cursor-pointer">
                                                <Checkbox :id="'FilterCategory-' + category.id" :value="category.id"
                                                    name="FilterInStock"
                                                    class="border-copy data-[state=checked]:bg-[var(--primary)] data-[state=checked]:text-primary-content"
                            ...
                            <div class="border-t-2 border-copy pt-4">
                                ...
                                <div class="px-1">
                                    <div class="flex justify-between text-sm text-copy-light mb-2 font-bold">
                                        <span class="cursor-pointer hover:text-primary-content"
                                            ...
                                            <div v-else class="border-2 border-copy bg-primary-light">
                                                <input ref="minPriceInput" type="number" v-model.number="form.min_price"
                                                    @blur="stopEditMinPrice" @keyup.enter="stopEditMinPrice"
                                        ...
                                        <span class="cursor-pointer hover:text-primary-content"
                                            ...
                                            <div v-else class="border-2 border-copy bg-primary-light">
                                                <input ref="maxPriceInput" type="number" v-model.number="form.max_price"
                                                    @blur="stopEditMaxPrice" @keyup.enter="stopEditMaxPrice"
                                    ...
                                    <input type="range" id="MinPrice" v-model.number="form.min_price" min="0" max="500"
                                        step="10"
                                    ...
                                    <input type="range" id="MaxPrice" v-model.number="form.max_price" min="0" max="500"
                                        step="10"
                ...
                <div class="lg:col-span-1">
                    <div
                        class="relative rounded-lg -m-0.5 flex items-center justify-between mb-6 p-3 bg-foreground border-2 border-copy">
                        <p class="text-sm text-copy-light font-medium">
                            Showing **{{ products.data.length }} of {{ products.total }}** results
                        </p>
                        <div class="flex items-center gap-3">
                            <label for="SortBy"
                                class="text-sm font-medium text-copy-light whitespace-nowrap hidden sm:block">Sort
                                by:</label>
                            <div class="border-2 rounded-lg border-copy bg-primary-light">
                                <select id="SortBy" v-model="form.sort"
                            ...
                            <button @click="filterOpen = true"
                                ...
                                aria-label="Toggle mobile filter panel" style="background-color: var(--primary);">
                    ...
                    <div :class="{ 'opacity-40 pointer-events-none': isLoading, 'opacity-100': !isLoading }"
                        ...
                        <div v-else class="text-center py-20 border-2 border-copy rounded-lg bg-error-light">
                            ...
                            <button @click="clearFilters"
                                class="mt-6 text-primary-content hover:text-primary-dark font-semibold underline transition">
                                Reset All Filters
                            </button>
                        </div>
                    </div>
                        ...
                        <ol class="flex justify-center gap-4 text-sm font-medium">
                            <li v-for="link in products.links" :key="link.label">
                                <div v-if="link.url"
                                    ...
                                    <button @click.prevent="paginate(link.url)"
                                ...
                                <span v-else
                                    class="block px-4 py-2 border-2 border-copy bg-border text-copy-lighter text-center leading-5 cursor-not-allowed"
                                    v-html="link.label.replace('Previous', '←').replace('Next', '→')"
    ...
    <Transition name="slide-fade">
        <div v-if="filterOpen" @click.self="filterOpen = false"
            class="lg:hidden fixed inset-0 z-50 bg-black bg-opacity-70 transition-opacity">
            <div
                class="fixed inset-y-0 right-0 w-80 bg-foreground p-6 shadow-2xl overflow-y-auto border-l-2 border-copy">
                <div class="flex justify-between items-center mb-6 border-b-2 border-copy pb-3">
                    ...
                    <button @click="filterOpen = false" class="text-copy-light hover:text-copy transition"
                        aria-label="Close mobile filter panel">
                ...
                <div class="space-y-6">
                    <div class="border-b-2 border-copy pb-4">
                        ...
                        <label for="MobileFilterInStock" class="inline-flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" id="MobileFilterInStock" v-model="form.in_stock"
                                class="size-5 border-2 border-copy text-primary-content focus:ring-primary-content" />
                    ...
                    <div class="border-b-2 border-copy pb-4">
                        ...
                        <div class="max-h-48 overflow-y-auto pr-2">
                            <ul class="space-y-2">
                                <li v-for="category in categories" :key="category.id">
                                    <label :for="'MobileFilterCategory-' + category.id"
                                        class="inline-flex items-center gap-2 cursor-pointer">
                                        <input type="checkbox" :id="'MobileFilterCategory-' + category.id"
                    ...
                    <div class="border-b-2 border-copy pb-4">
                        ...
                        <div class="px-1">
                            ...
                            <input type="range" v-model.number="form.min_price" min="0" max="500" step="10"
                                class="w-full h-1 bg-border rounded-lg appearance-none cursor-pointer [&::-webkit-slider-thumb]:bg-primary-content mt-2" />
