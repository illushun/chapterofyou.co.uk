...
const fmtK = (v: unknown) =>
    n(v) >= 1000 ? `£${(n(v) / 1000).toFixed(1)}k` : fmt(v);
const fmtNum = (v: unknown) => n(v).toLocaleString();

const pctUp = (v: unknown) => n(v) >= 0;
const pctColour = (v: unknown) =>
    n(v) >= 0 ? 'var(--adm-success)' : 'var(--adm-danger)';
...
const donutSegments = computed(() => {
    ...
    const total =
        Object.values(breakdown).reduce((a, b) => n(a) + n(b), 0) || 1;
...
const totalOrders = computed(() =>
    Object.values(props.stats.status_breakdown).reduce(
        (a, b) => n(a) + n(b),
        0,
    ),
);
...
const returnRate = computed(() => {
    const total =
        n(props.stats.customers.new_30) + n(props.stats.customers.returning_30);
    if (!total) return 0;
        ...
        <div class="db-kpi-grid">
            ...
            <div class="adm-card db-kpi-card">
                <div class="db-kpi-top">
                    ...
                    <span
                        class="db-kpi-pct"
                        :style="{ color: pctColour(stats.pct_change.revenue) }"
                    >
                        <ArrowUp
                            v-if="pctUp(stats.pct_change.revenue)"
                            :size="11"
                            :stroke-width="3"
                        />
                        <ArrowDown v-else :size="11" :stroke-width="3" />
                        {{ Math.abs(n(stats.pct_change.revenue)) }}%
                    </span>
                </div>
                <p class="db-kpi-val">
                    {{ fmtK(stats.current_period.revenue) }}
                </p>
            ...
            <div class="adm-card db-kpi-card">
                <div class="db-kpi-top">
                    ...
                    <span
                        class="db-kpi-pct"
                        :style="{ color: pctColour(stats.pct_change.orders) }"
                    >
                        <ArrowUp
                            v-if="pctUp(stats.pct_change.orders)"
                            :size="11"
                            :stroke-width="3"
                        />
                        <ArrowDown v-else :size="11" :stroke-width="3" />
                        {{ Math.abs(n(stats.pct_change.orders)) }}%
                    </span>
                </div>
                <p class="db-kpi-val">{{ stats.current_period.orders }}</p>
                <p class="db-kpi-label">Orders</p>
                <p class="db-kpi-sub">
                    Avg {{ fmt(stats.current_period.avg_order) }} per order
                </p>
            </div>
            ...
            <div class="adm-card db-kpi-card">
                <div class="db-kpi-top">
                    ...
                    <span
                        class="db-kpi-badge"
                        :style="
                            n(stats.cart_abandonment.abandonment_rate) > 70
                                ? 'background:var(--adm-danger-bg);color:var(--adm-danger)'
                ...
                <p class="db-kpi-val">
                    {{ stats.cart_abandonment.active_carts }}
                </p>
                <p class="db-kpi-label">Abandoned Carts</p>
                <p class="db-kpi-sub">
                    {{ stats.cart_abandonment.completed_orders }} carts
                    converted to orders
                </p>
            </div>
            ...
            <div class="adm-card db-kpi-card">
                ...
                <p class="db-kpi-sub">
                    {{ stats.customers.new_7 }} this week ·
        ...
        <div class="db-charts-grid">
            ...
            <div class="adm-card adm-card--flush">
                ...
                <div class="db-bar-chart">
                    ...
                    <p
                        v-if="!barChart.some((b) => b.revenue > 0)"
                        class="db-empty-note"
                    >
                        No revenue data yet.
                    </p>
                ...
                <div class="db-donut-wrap">
                    <svg viewBox="0 0 42 42" class="db-donut-svg">
                        ...
                        <text
                            ...
                            text-anchor="middle"
                            dominant-baseline="middle"
                            style="font-size: 3px; fill: var(--adm-ink-dim)"
                        >
                            orders
                        </text>
                    </svg>
                ...
                <div class="db-donut-legend">
                    ...
                    >
                        <div class="db-legend-left">
                            ...
                            ></span>
                            <span class="db-legend-label">{{
                                seg.status
                            }}</span>
                        </div>
                        <span class="db-legend-val">{{ seg.count }}</span>
                    </div>
                    <p v-if="!donutSegments.length" class="db-empty-note">
                        No orders yet.
                    </p>
                </div>
            </div>
            ...
            <div class="adm-card">
                ...
                <div class="db-retention-wrap">
                    ...
                    <div class="db-retention-bar">
                        <div
                            class="db-retention-seg db-retention-seg--new"
                            :style="{ width: 100 - returnRate + '%' }"
                            :title="`New: ${stats.customers.new_30}`"
                        ></div>
                        <div
                            class="db-retention-seg db-retention-seg--return"
                            :style="{ width: returnRate + '%' }"
                            :title="`Returning: ${stats.customers.returning_30}`"
                        ></div>
                    </div>
            ...
            <div class="adm-card">
                ...
                <div class="db-stat-list">
                    <div class="db-stat-row">
                        <span class="db-stat-lbl"
                            >Orders with discount code</span
                        >
                        <span class="db-stat-val adm-badge--lav">{{
                            stats.vouchers.orders_with_voucher
                        }}</span>
                    </div>
                    <div class="db-stat-row">
                        ...
                        <span
                            class="db-stat-val"
                            style="color: var(--adm-success); font-weight: 700"
                            >{{
                                fmt(stats.vouchers.total_discount_given)
                            }}</span
                        >
                    </div>
                    <div class="db-stat-row">
                        <span class="db-stat-lbl"
                            >Gift vouchers sold (30d)</span
                        >
                        <span class="db-stat-val adm-badge--blush">{{
                            stats.vouchers.gift_vouchers_sold
                        }}</span>
                    </div>
                    <div class="db-stat-row">
                        <span class="db-stat-lbl"
                            >Gift vouchers awaiting fulfilment</span
                        >
                        <span
                            class="db-stat-val"
                            :class="
                                stats.vouchers.gift_vouchers_pending > 0
                                    ? 'adm-badge--warn'
                                    : 'adm-badge--on'
                            "
                        >
                ...
                <Link
                    v-if="stats.vouchers.gift_vouchers_pending > 0"
                    :href="route('admin.gift-vouchers.index')"
            ...
            <div class="adm-card">
                ...
                <div class="db-reviews-centre">
                    <div class="db-avg-rating">
                        <span class="db-avg-num">{{
                            n(stats.reviews.average_rating).toFixed(1)
                        }}</span>
                        <div class="db-stars">
                            <Star
                                v-for="i in 5"
                                ...
                                :class="
                                    i <=
                                    Math.round(n(stats.reviews.average_rating))
                                        ? 'db-star--filled'
                                        : ''
                                "
                                :fill="
                        ...
                        <span class="db-avg-sub"
                            >{{ stats.reviews.total_approved }} approved
                            reviews</span
                        >
                    </div>
                </div>
                <div
                    v-if="stats.reviews.pending_approval > 0"
                    class="db-pending-reviews"
                >
                    <svg
                        width="13"
                        height="13"
                    ...
                    <span
                        >{{ stats.reviews.pending_approval }} review{{
                            stats.reviews.pending_approval !== 1 ? 's' : ''
                        }}
                        awaiting approval</span
                    >
                    <Link
                        :href="route('admin.reviews.index')"
                        class="db-pending-link"
                        >Review <ArrowRight :size="12" :stroke-width="2.5"
                    /></Link>
                </div>
                <p v-else class="db-empty-note" style="margin-top: 0.75rem">
            ...
            <div class="adm-card adm-card--flush">
                ...
                <div class="db-list">
                    ...
                    <div
                        ...
                        class="db-list-row"
                    >
                        <span class="db-rank" :class="`db-rank--${i + 1}`">{{
                            i + 1
                        }}</span>
                        ...
                        <div class="db-list-right">
                            <p class="db-list-primary">
                                {{ p.units_sold }} sold
                            </p>
                            <p class="db-list-secondary">
                                {{ fmt(p.revenue) }}
                            </p>
                        </div>
                    </div>
            ...
            <div class="adm-card adm-card--flush">
                <div class="db-card-header">
                    ...
                    <Link
                        :href="route('admin.orders.index')"
                        class="db-view-all"
                        >View all <ArrowRight :size="12" :stroke-width="2.5"
                    /></Link>
                </div>
                <div class="db-list">
                    <p
                    ...
                    <Link
                        ...
                        class="db-list-row db-list-row--link"
                    >
                        <div class="db-list-info">
            ...
            <div class="adm-card adm-card--flush">
                <div class="db-card-header">
                    ...
                    <Link
                        ...
                        class="db-view-all"
                        >Manage <ArrowRight :size="12" :stroke-width="2.5"
                    /></Link>
                </div>
                <div class="db-journal-summary">
                    <div class="db-journal-stat">
                        <span class="db-journal-num">{{
                            stats.journal.total_posts
                        }}</span>
                    ...
                    <div class="db-journal-stat">
                        <span class="db-journal-num">{{
                            fmtNum(stats.journal.total_views)
                        }}</span>
                ...
                <div class="db-list">
                    <p
                        v-if="!stats.journal.top_posts?.length"
                        class="db-empty-note"
                        style="padding: 1rem"
                    >
                    ...
                    <a
                        v-for="post in stats.journal.top_posts"
                        :key="post.id"
        ...
        <div class="adm-card" style="margin-top: 1.5rem">
            ...
            <div class="db-product-health">
                <div class="db-ph-stat">
                    <span class="db-ph-num" style="color: var(--adm-success)">{{
                ...
                <div class="db-ph-stat">
                    <span class="db-ph-num" style="color: var(--adm-warning)">{{
                    ...
                    <div class="db-ph-bar">
                        <div
                            class="db-ph-fill"
                            style="background: var(--adm-warning)"
                            :style="{
                                width: stats.products.total
                                    ? (stats.products.out_of_stock /
                                          stats.products.total) *
                ...
                <div class="db-ph-stat">
                    <span
                        ...
                        >{{ stats.products.low_stock }}</span
                    >
                    <span class="db-ph-lbl">Low Stock (≤5)</span>
                    <div class="db-ph-bar">
                        <div
                            class="db-ph-fill"
                            style="background: var(--adm-stamp-deep)"
                            :style="{
                                width: stats.products.total
                ...
                <div class="db-ph-stat">
                    <span class="db-ph-num" style="color: var(--adm-ink-dim)">{{
                    ...
                    <div class="db-ph-bar">
                        <div
                            class="db-ph-fill"
                            style="background: var(--adm-ink-dim)"
                            :style="{
                                width: stats.products.total
                                    ? (stats.products.disabled /
                                          stats.products.total) *
            ...
            >
                ...
                <Link
                    ...
                    "
                >
                    <TriangleAlert :size="13" :stroke-width="2.5" />
...
.db-kpi-icon--lav {
    background: var(--adm-paper-sunk, #f3f0ff);
    color: var(--adm-charcoal);
}

.db-kpi-icon--blush {
    background: var(--adm-stamp-dim, #fff0f2);
    color: var(--adm-stamp-deep);
}

.db-kpi-icon--peach {
    background: var(--adm-warning-bg, #fff7f0);
...
.db-pending-reviews {
    ...
    background: var(--adm-danger-bg);
    border: 1px dashed var(--adm-danger-line);
    color: var(--adm-danger);
    font-size: 0.82rem;
}
