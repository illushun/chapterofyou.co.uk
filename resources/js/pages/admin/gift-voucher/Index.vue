...
function markDispatched(id: number) {
    router.post(
        route('admin.gift-vouchers.dispatch', id),
        {},
        {
            preserveScroll: true,
        },
...
function resendEmail(id: number) {
    router.post(
        route('admin.gift-vouchers.resend', id),
        {},
        {
            preserveScroll: true,
        },
    ...
    <AdminLayout>
        <div class="adm-header">
            <div>
                ...
                <p class="adm-sub">
                    Manage purchased gift vouchers and track fulfilment.
                </p>
            </div>
        </div>
        ...
        <div class="adm-stats" style="margin-bottom: 1.5rem">
            <div class="adm-stat--blush">
                <div class="adm-stat-val">
                    {{ giftVouchers.meta?.total ?? giftVouchers.data.length }}
                </div>
            ...
            <div class="adm-stat--lav">
                <div class="adm-stat-val">
                    {{
                        giftVouchers.data.filter((g) => !g.fulfilled_at).length
                    }}
                </div>
            ...
            <div class="adm-stat--sage">
                <div class="adm-stat-val">
                    {{
                        giftVouchers.data.filter(
                            (g) => g.delivery_type === 'physical',
                        ).length
                    }}
                </div>
        ...
        <div class="adm-table-wrap">
            <table class="adm-table">
                ...
                <tbody>
                    <tr
                        v-for="gv in giftVouchers.data"
                        :key="gv.id"
                        class="adm-row"
                    >
                        <td class="adm-td adm-td--mono">#{{ gv.order?.id }}</td>
                        <td class="adm-td">
                            {{ gv.order?.first_name }} {{ gv.order?.last_name }}
                        </td>
                        <td class="adm-td">
                            <div>{{ gv.recipient_name }}</div>
                            <div
                                v-if="gv.recipient_email"
                                style="
                                    font-size: 0.75rem;
                        ...
                        <td class="adm-td">
                            <span
                                v-if="gv.fulfilled_at"
                                class="adm-badge adm-badge--on"
                                >Fulfilled</span
                            >
                            <span v-else class="adm-badge adm-badge--warn"
                                >Pending</span
                            >
                        </td>
                        <td class="adm-td adm-td--actions">
                            ...
                            <button
                                v-if="
                                    gv.delivery_type === 'physical' &&
                                    !gv.fulfilled_at
                                "
                                @click="markDispatched(gv.id)"
                            ...
                            <!-- E-voucher: resend -->
                            <button
                                v-if="gv.delivery_type === 'email'"
                                @click="resendEmail(gv.id)"
