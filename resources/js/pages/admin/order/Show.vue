...
function sendDispatch() {
    ...
    router.post(
        route('admin.orders.dispatch-email', props.order.id),
        {
            tracking_url: trackingUrl.value || null,
        },
        {
...
function resendConfirmation() {
    ...
    router.post(
        route('admin.orders.resend-confirmation', props.order.id),
        {},
        {
            preserveScroll: true,
            onFinish: () => {
...
const CHECKLIST: ChecklistGroup[] = [
    {
        ...
        items: [
            ...
            {
                id: 'eq-tools',
                label: 'Get jugs / beakers / stirrers / pipettes / funnel',
            },
    ...
    },
    {
        ...
        items: [
            {
                id: 'mk-batch',
                label: 'Create Batch Sheet (1 per product) in Batch Sheets',
            },
        ],
    },
    {
        ...
        items: [
            ...
            {
                id: 'pk-warning',
                label: 'Get tactical warning label for diffuser',
            },
            { id: 'pk-vellum', label: 'Get vellum paper to line shipping box' },
            { id: 'pk-shred', label: 'Get shredded paper' },
            {
                id: 'pk-cards',
                label: 'Get reed diffuser care card / business card / thank you card',
            },
            { id: 'pk-sweets', label: 'Get sweets / chocolates' },
            {
                id: 'pk-sticker-lg',
                label: 'Get large logo sticker for front of diffuser box',
            },
            {
                id: 'pk-sticker-sm',
                label: 'Get small logo sticker for back of diffuser box',
            },
...
function toggleGroup(group: ChecklistGroup) {
    ...
    group.items.forEach((i) =>
        allChecked ? checked.delete(i.id) : checked.add(i.id),
    );
    persist();
}
...
function resetChecklist() {
    if (!confirm('Reset checklist for this order? All ticks will be cleared.'))
        return;
    checked.clear();
    persist();
}
    ...
    <AdminLayout>
        ...
        <div class="adm-header">
            ...
            <span
                ...
                style="font-size: 0.82rem; padding: 0.3rem 0.9rem"
            >
                {{ order.status }}
            </span>
        </div>
        ...
        <div class="os-layout">
            ...
            <div class="os-main">
                ...
                <section class="adm-card adm-card--flush">
                    ...
                    <div
                        ...
                        :class="{
                            'os-item-row--gv':
                                item.product.mpn === 'GIFT-VOUCHER',
                        }"
                    >
                        <div class="os-item-info">
                            <p class="os-item-name">
                                <Gift
                                    v-if="item.product.mpn === 'GIFT-VOUCHER'"
                                    :size="13"
                                    :stroke-width="2"
                                    style="
                                        display: inline;
                            ...
                            >
                                ...
                                >
                                    For:
                                    <strong>{{
                                        giftVoucherOrder.recipient_name
                                    }}</strong>
                                    <span class="os-sep">·</span>
                                    {{
                                        giftVoucherOrder.delivery_type ===
                                        'email'
                                            ? 'E-Voucher'
                                            : 'Physical Voucher'
                                    }}
                                    <template
                                        v-if="giftVoucherOrder.recipient_email"
                                    >
                                        <span class="os-sep">·</span>
                        ...
                        <div class="os-item-right">
                            <p class="os-item-price">
                                {{ fmtCurrency(item.product_total) }}
                            </p>
                ...
                <section class="adm-card adm-card--flush">
                    ...
                    <div class="cl-header">
                        <div class="cl-header-left">
                            ...
                            <span
                                class="cl-progress-pill"
                                :class="allDone ? 'cl-progress-pill--done' : ''"
                            >
                    ...
                    <div class="cl-bar-track">
                        <div
                            class="cl-bar-fill"
                            :class="allDone ? 'cl-bar-fill--done' : ''"
                    ...
                    >
                        ...
                        <button
                            ...
                            :class="`cl-group-header--${group.colour}`"
                            @click="toggleCollapse(group.id)"
                        >
                            <div class="cl-group-left">
                                <span
                                    class="cl-group-dot"
                                    :class="`cl-group-dot--${group.colour}`"
                                ></span>
                                <span class="cl-group-label">{{
                                    group.label
                                }}</span>
                                <span
                                    class="cl-group-count"
                                    :class="
                        ...
                        <div v-if="!collapsed.has(group.id)" class="cl-items">
                            <label
                                ...
                                class="cl-item"
                                :class="{
                                    'cl-item--checked': checked.has(item.id),
                                }"
                            >
                                <span
                                    class="cl-checkbox-wrap"
            ...
            <div class="os-sidebar">
                ...
                <section class="adm-card adm-card--sticky">
                    ...
                    <div class="os-totals">
                        ...
                        <div class="os-total-row">
                            ...
                            <span>{{
                                Number(order.shipping_total) === 0
                                    ? 'FREE'
                                    : fmtCurrency(order.shipping_total)
                            }}</span>
                        </div>
                        ...
                        <div
                            ...
                            class="os-total-row os-total-row--discount"
                        >
                            <span>Discount</span>
                            <span
                                >−{{
                                    fmtCurrency(order.voucher_discount)
                                }}</span
                            >
                        </div>
                        <div class="os-total-row os-total-grand">
                            <span>Grand Total</span>
                            <span class="os-grand-val">{{
                ...
                <section class="adm-card">
                    ...
                    <div class="adm-field">
                        <label class="adm-label" for="status-select"
                            >New Status</label
                        >
                        <select
                            id="status-select"
                ...
                <section class="adm-card">
                    ...
                    <div class="adm-field">
                        ...
                        />
                        <p class="os-hint">
                            Used in both dispatch email and status update.
                        </p>
                    </div>
                    <button
                        @click="sendDispatch"
                ...
                <section class="adm-card">
                    ...
                    <div class="os-links">
                        <a
                            :href="
                                route('admin.batch-sheets.create', {
                                    order_id: order.id,
                                })
                            "
                            class="os-link-item"
                        ...
                        <a
                            v-if="order.user_id"
                            :href="route('admin.users.show', order.user_id)"
                            class="os-link-item"
                        >
                            <svg
                                width="14"
                        ...
                        <a
                            :href="route('admin.orders.index')"
                            class="os-link-item os-link-item--muted"
                        >
                            <svg
                                width="14"
                                height="14"
...
.os-link-item:hover {
    ...
    border-color: var(--adm-stamp-dim);
}
...
.cl-done-banner {
    ...
    font-size: 0.82rem;
    font-weight: 700;
    border-bottom: 1px solid var(--adm-success-line);
}
