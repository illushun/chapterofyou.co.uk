...
<template>
    <AdminLayout>
        ...
        <div class="flex justify-between items-start mb-6 border-b-2 border-copy pb-2">
            <div>
                <h2 class="text-3xl font-black">Order #{{ order.id }}</h2>
                <p class="text-copy-light">Placed on {{ formatDate(order.created_at) }}</p>
            </div>
            <span :class="['px-4 py-1.5 rounded-full text-lg font-bold uppercase border-2', getStatusClasses(order.status)]">
                {{ order.status }}
            </span>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="rounded-xl border-2 border-copy bg-[var(--primary-content)] shadow-xl">
                    <div class="relative rounded-xl -m-0.5 border-2 border-copy bg-foreground p-6">
                        <h3 class="text-xl font-bold text-copy mb-4 border-b-2 border-copy-light pb-2">Order Items ({{ order.items.length }})</h3>
                        <div v-for="item in order.items" :key="item.id" class="flex items-center justify-between py-3 border-b border-copy-light/50 last:border-b-0">
                            <div class="flex flex-col">
                                <span class="font-semibold text-copy">{{ item.product.name }}</span>
                                <span class="text-xs text-copy-light">MPN: {{ item.product.mpn }} | Unit Price: {{ formatCurrency(item.product_cost) }}</span>
                            </div>
                ...
                <div class="rounded-xl border-2 border-copy bg-[var(--primary-content)] shadow-xl">
                    <div class="relative rounded-xl -m-0.5 border-2 border-copy bg-foreground p-6">
                        <h3 class="text-xl font-bold text-copy mb-4 border-b-2 border-copy-light pb-2">Customer Details</h3>
                        <div class="grid grid-cols-2 gap-4 text-copy">
                            <div><span class="font-semibold">Name:</span> {{ order.first_name }} {{ order.last_name }}</div>
                            <div><span class="font-semibold">Email:</span> <a :href="`mailto:${order.email}`" class="text-primary hover:underline">{{ order.email }}</a></div>
                            <div><span class="font-semibold">Telephone:</span> {{ order.telephone || 'N/A' }}</div>
                            <div><span class="font-semibold">User Account:</span>
                                <Link v-if="order.user" :href="route('admin.users.show', order.user_id)" class="text-primary hover:underline">
                ...
                <div class="rounded-xl border-2 border-copy bg-[var(--primary-content)] shadow-xl">
                    <div class="relative rounded-xl -m-0.5 border-2 border-copy bg-foreground p-6">
                        <h3 class="text-xl font-bold text-copy mb-4 border-b-2 border-copy-light pb-2">Shipping & Billing</h3>
            ...
            <div class="lg:col-span-1 space-y-6">
                <div class="sticky top-8 rounded-xl border-2 border-copy bg-[var(--primary-content)] shadow-xl">
                    <div class="relative rounded-xl -m-0.5 border-2 border-copy bg-foreground p-6">
                        <h3 class="text-2xl font-black text-copy mb-4 border-b-2 border-copy-light pb-3">Financial Summary</h3>
                        ...
                        <div class="mt-6 pt-4 border-t-2 border-copy-light flex justify-between items-center">
                            ...
                            <span class="text-4xl font-black text-primary">{{ formatCurrency(order.grand_total) }}</span>
                        </div>
                        ...
                        <div class="mt-4 pt-4 border-t border-copy-light text-sm">
                             ...
                             <p><span class="font-semibold">Intent ID:</span> <span class="text-xs break-all">{{ order.payment_intent_id }}</span></p>
                        </div>
