    ...
    <AdminLayout>
        ...
        <div class="adm-header">
            ...
            <button @click="showAdd = !showAdd" class="adm-btn adm-btn--primary adm-btn--sm">
                <svg v-if="!showAdd" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor"
        ...
        <div class="adm-form-grid pcd-grid">
                    ...
                    <form v-else @submit.prevent="submitAdd">
                        <div class="pcd-add-row">
                            <div class="adm-field" style="flex:3">
                                ...
                                <p v-if="addForm.errors.cost_item_id" class="adm-err">{{ addForm.errors.cost_item_id }}
                                </p>
                            </div>
                            <div class="adm-field" style="flex:1">
                                ...
                                <input v-model="addForm.qty_per_unit" type="number" step="0.0001" min="0.0001"
                                    class="adm-input" :class="{ 'adm-input--err': addForm.errors.qty_per_unit }"
                                    placeholder="1" />
                                <p v-if="addForm.errors.qty_per_unit" class="adm-err">{{ addForm.errors.qty_per_unit }}
                                </p>
                            </div>
                        ...
                        <div style="display:flex;justify-content:flex-end;gap:.6rem;margin-top:.85rem">
                            ...
                            <button type="submit" class="adm-btn adm-btn--primary adm-btn--sm"
                                :disabled="addForm.processing">Add</button>
                        </div>
                    </form>
                </div>
                                ...
                                <tr v-for="item in costItems" :key="item.id" class="adm-row">
                                    <td class="adm-td">
                                        ...
                                        <div style="display:flex;gap:.4rem;align-items:center;margin-top:.2rem">
                                            <span class="adm-badge"
                                                :class="categoryBadge[item.category] ?? 'adm-badge--off'"
                                        ...
                                        <template v-if="editingQty === item.id">
                                            <div class="pcd-qty-edit">
                                                <input v-model="qtyValue" type="number" step="0.0001" min="0.0001"
                                                    ...
                                                    @keydown.enter="saveQty(item)" @keydown.escape="cancelEditQty" />
                                                <button @click="saveQty(item)" class="adm-action adm-action--edit"
                                                    title="Save">✓</button>
                                                <button @click="cancelEditQty" class="adm-action adm-action--del"
                                                    title="Cancel">✕</button>
                                            </div>
                                        </template>
                                    ...
                                    <td class="adm-td adm-td--actions">
                                        <button @click="removeItem(item)"
                                            class="adm-action adm-action--del">Remove</button>
                                    </td>
                                </tr>
                            </tbody>
                            <tfoot v-if="costItems.length">
                                <tr class="pcd-totals-row">
                                    <td colspan="3" class="adm-td"
                                        style="text-align:right;font-weight:700;font-size:.82rem;letter-spacing:.04em;text-transform:uppercase;color:var(--bb-muted)">
                                        Total COGS
                                    </td>
                    ...
                    <div class="adm-mob-list">
                        <div v-for="item in costItems" :key="item.id" class="adm-mob-card">
                            <div class="pcd-mob-head">
                                ...
                                <p class="pcd-contribution" style="font-size:1rem">{{ fmtCurrency(item.contribution) }}
                                </p>
                            </div>
                            ...
                            <div style="display:flex;gap:.5rem">
                                <button @click="startEditQty(item)" class="adm-btn adm-btn--ghost adm-btn--sm">Edit
                                    Qty</button>
                                <button @click="removeItem(item)"
                                    class="adm-btn adm-btn--danger adm-btn--sm">Remove</button>
                            </div>
                        </div>
            ...
            <div class="adm-form-right">
                ...
                <div class="adm-card adm-card--sm">
                    <img v-if="coverImage" :src="`/storage/${coverImage}`" :alt="product.name" class="pcd-cover" />
                    ...
                    <div v-else-if="margin !== null && parseFloat(margin) < 30" class="adm-flash adm-flash--warn"
                        style="margin-top:1rem;font-size:.78rem;margin-bottom:0">
                ...
                <div v-if="costItems.length" class="adm-card adm-card--sm">
                    ...
                    <div class="pcd-summary-row">
                        ...
                        <div class="pcd-target-input-wrap">
                            <input id="targetMargin" v-model.number="targetMargin" type="number" min="1" max="99"
                                step="1" class="adm-input adm-input--sm pcd-target-input" />
                    ...
                    <div v-if="idealSellingPrice !== null" class="pcd-summary-row" style="margin-top:.1rem">
                        ...
                        <span
                            :style="idealSellingPrice > product.cost ? 'color:var(--bb-red);font-size:.8rem;font-weight:600' : 'color:var(--bb-sage-d);font-size:.8rem;font-weight:600'">
                            {{ idealSellingPrice > product.cost ? '+' : '' }}{{ fmtCurrency(idealSellingPrice -
                            product.cost) }}
                        </span>
                    </div>
                </div>
