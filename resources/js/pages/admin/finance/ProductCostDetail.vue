...
import { Calculator, Check, X } from 'lucide-vue-next';
...
const selectedItem = computed(
    () =>
        props.allItems.find(
            (i) => i.id === parseInt(String(addForm.cost_item_id)),
        ) ?? null,
);
...
const addPreview = computed(() => {
    ...
    return (
        selectedItem.value.unit_cost * parseFloat(String(addForm.qty_per_unit))
    );
});
...
const saveQty = (item: AttachedItem) => {
    router.put(
        route('admin.finance.products.costs.update', [
            props.product.id,
            item.id,
        ]),
        {
...
const removeItem = (item: AttachedItem) => {
    ...
    router.delete(
        route('admin.finance.products.costs.remove', [
            props.product.id,
            item.id,
        ]),
    );
...
const idealSellingPrice = computed(() => {
    if (
        !props.costItems.length ||
        targetMargin.value <= 0 ||
        targetMargin.value >= 100
    )
        return null;
        ...
        <div class="adm-header">
            <div>
                <div class="adm-breadcrumb">
                    ...
                    <Link :href="route('admin.finance.products')"
                        >Product Costs</Link
                    >
                    <span class="adm-breadcrumb-sep">/</span>
            ...
            <div class="adm-form-left">
                ...
                <div v-if="showAdd" class="adm-card adm-card--sm">
                    ...
                    >
                        <p class="adm-empty-sub">
                            All cost items are already added.
                        </p>
                        <Link
                        ...
                        <div class="pcd-add-row">
                            <div class="adm-field" style="flex: 3">
                                <label class="adm-label"
                                    >Item <span class="pcd-req">*</span></label
                                >
                                <select
                                    v-model="addForm.cost_item_id"
                                    class="adm-select"
                                    :class="{
                                        'adm-select--err':
                                            addForm.errors.cost_item_id,
                                    }"
                                >
                                ...
                                <p
                                    v-if="addForm.errors.cost_item_id"
                                    class="adm-err"
                                >
                                    {{ addForm.errors.cost_item_id }}
                                </p>
                            ...
                            <div class="adm-field" style="flex: 1">
                                <label class="adm-label"
                                    >Qty / product
                                    <span class="pcd-req">*</span></label
                                >
                                <input
                                    v-model="addForm.qty_per_unit"
                                    type="number"
                                    step="0.0001"
                                    min="0.0001"
                                    class="adm-input"
                                    :class="{
                                        'adm-input--err':
                                            addForm.errors.qty_per_unit,
                                    }"
                                    placeholder="1"
                                />
                                <p
                                    v-if="addForm.errors.qty_per_unit"
                                    class="adm-err"
                                >
                                    {{ addForm.errors.qty_per_unit }}
                                </p>
                ...
                <div class="adm-card adm-card--flush">
                        ...
                        <table class="adm-table">
                            ...
                            <tbody>
                                ...
                                >
                                    ...
                                    <td
                                        class="adm-td adm-td--right adm-td--price pcd-contribution"
                                    >
                            ...
                            <tfoot v-if="costItems.length">
                                <tr class="pcd-totals-row">
                                    ...
                                    <td class="adm-td adm-td--right pcd-total">
                    ...
                    <div class="adm-mob-list">
                        ...
                        <div
                            v-if="costItems.length"
                            class="adm-mob-card"
                            style="background: var(--adm-paper)"
                        >
                            <div
                                style="
                                    display: flex;
                    ...
                    <div v-if="!costItems.length" class="adm-empty">
                        ...
                        <p class="adm-empty-sub">
                            Add the supplies that go into this product.
                        </p>
                        <button
                            @click="showAdd = true"
            ...
            <div class="adm-form-right">
                ...
                <div class="adm-card adm-card--sm">
                    ...
                    <div class="pcd-summary-row">
                        ...
                        <strong
                            :style="
                                totalCost > 0 ? 'color:var(--adm-danger)' : ''
                            "
                        >
                            {{ fmtCurrency(totalCost) }}
                        </strong>
                    ...
                    <div class="pcd-summary-row" style="margin-top: 0.3rem">
                        <span>Margin</span>
                    ...
                    <div
                        v-else-if="margin !== null && parseFloat(margin) < 30"
                        class="adm-flash adm-flash--warn"
                        style="
                            margin-top: 1rem;
                            font-size: 0.78rem;
                            margin-bottom: 0;
                        "
                    >
                        Margin is below 30%. Consider reviewing your costs or
                        pricing.
                    </div>
                </div>
                ...
                <div v-if="costItems.length" class="adm-card adm-card--sm">
                    ...
                    <div class="pcd-summary-row">
                        <label class="pcd-target-label" for="targetMargin"
                            >Target margin</label
                        >
                        <div class="pcd-target-input-wrap">
                            <input
                                ...
                                max="99"
                                step="1"
                                class="adm-input adm-input--sm pcd-target-input"
                            />
                        ...
                        <strong class="pcd-ideal-price">
                            {{
                                idealSellingPrice !== null
                                    ? fmtCurrency(idealSellingPrice)
                                    : '-'
                            }}
                        </strong>
                    </div>
                    <div
                        v-if="idealSellingPrice !== null"
                        class="pcd-summary-row"
                    ...
                    >
                        <span
                            style="
                                ...
                                color: var(--adm-ink-dim);
                            "
                            >vs. current price</span
                        >
                        <span
                            :style="
                                idealSellingPrice > product.cost
                                    ? 'color:var(--adm-danger);font-size:.8rem;font-weight:600'
                                    : 'color:var(--adm-success);font-size:.8rem;font-weight:600'
                            "
                        >
                            {{ idealSellingPrice > product.cost ? '+' : ''
                            }}{{
                                fmtCurrency(idealSellingPrice - product.cost)
                            }}
                        </span>
                    </div>
                </div>
                ...
                <div v-if="costItems.length" class="adm-card adm-card--sm">
                    ...
                    <template v-for="(label, key) in categories" :key="key">
                        <div
                            v-if="costItems.some((i) => i.category === key)"
                            class="pcd-summary-row"
                        >
                            <span
                                class="adm-badge"
                                :class="categoryBadge[key] ?? 'adm-badge--off'"
                                style="font-size: 0.68rem"
                                >{{ label }}</span
                            >
                            <strong>{{
                                fmtCurrency(categoryTotal(key))
                            }}</strong>
                        </div>
                    </template>
                </div>
