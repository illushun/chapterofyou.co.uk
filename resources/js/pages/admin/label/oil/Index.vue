...
function tabFor(id: number): 'hazards' | 'components' | 'sds' {
    return activeTab.value[id] ?? 'hazards';
}
function toggleOil(id: number) {
    openOilId.value = openOilId.value === id ? null : id;
}
function setTab(id: number, tab: 'hazards' | 'components' | 'sds') {
    activeTab.value[id] = tab;
}
function startEditHazard(h: OilHazard) {
    editingHazard.value = h.id;
    hazardEdits.value[h.id] = { ...h };
}
...
async function deleteHazard(oilId: number, hazardId: number) {
    ...
    await axios.delete(
        route('admin.oils.hazards.destroy', { oil: oilId, hazard: hazardId }),
    );
...
async function deleteComponent(oilId: number, componentId: number) {
    ...
    await axios.delete(
        route('admin.oils.components.destroy', {
            oil: oilId,
...
async function submitUpdateHazard(oilId: number, hazardId: number) {
    await axios.put(
        route('admin.oils.hazards.update', { oil: oilId, hazard: hazardId }),
        hazardEdits.value[hazardId],
    );
    editingHazard.value = null;
    router.reload({ only: ['oils'] });
}
async function submitAddHazard(oilId: number) {
    await axios.post(
        route('admin.oils.hazards.store', { oil: oilId }),
        newHazard.value,
    );
    ...
    <AdminLayout>
        ...
        <div class="adm-header">
            <div>
                ...
                <p class="adm-sub">
                    Manage fragrance &amp; essential oils and their SDS data
                </p>
            </div>
            <button
        ...
        <div v-if="showAddForm" class="adm-card adm-card--sm oi-add-card">
            ...
            <div class="oi-add-grid">
                ...
                <div class="adm-field">
                    ...
                    <input
                        v-model="newOil.cas_primary"
                        type="text"
                        class="adm-input"
                        placeholder="e.g. 8000-28-0"
                    />
                </div>
            </div>
            <div
                ...
                "
            >
                <button class="adm-btn adm-btn--primary" @click="submitAddOil">
                    Save Oil
                </button>
            </div>
        </div>
        ...
        <div class="oi-search-wrap">
            ...
            <input
                v-model="searchQuery"
                type="text"
                class="oi-search-input"
                placeholder="Search by name or supplier…"
            />
            <button
                v-if="searchQuery"
                @click="searchQuery = ''"
                class="oi-search-clear"
                aria-label="Clear"
            >
                <svg
                    width="12"
        ...
        <div class="oi-list">
            <div v-if="!filteredOils.length" class="adm-empty">
                <div class="adm-empty-icon">
                    <FlaskConical :size="28" :stroke-width="1.5" />
                </div>
                <p class="adm-empty-title">No oils found</p>
                <p class="adm-empty-sub">
                    Try a different search or add a new oil above.
                </p>
            </div>
                ...
                <div class="oi-row-head" @click="toggleOil(oil.id)">
                    ...
                    <div class="oi-row-badges">
                        ...
                        <span class="adm-badge adm-badge--lav">
                            {{ oil.hazards.length }} hazard{{
                                oil.hazards.length !== 1 ? 's' : ''
                            }}
                        </span>
                        <svg
                            class="oi-chevron"
                            :class="{
                                'oi-chevron--open': openOilId === oil.id,
                            }"
                            width="14"
                            height="14"
                            viewBox="0 0 24 24"
                ...
                <div v-if="openOilId === oil.id" class="oi-detail">
                    ...
                    <div class="oi-tabs">
                        <button
                            class="oi-tab"
                            :class="{
                                'oi-tab--active': tabFor(oil.id) === 'hazards',
                            }"
                            @click="setTab(oil.id, 'hazards')"
                        >
                            Section 2: Hazards
                        </button>
                        <button
                            ...
                            :class="{
                                'oi-tab--active':
                                    tabFor(oil.id) === 'components',
                            }"
                            @click="setTab(oil.id, 'components')"
                        >
                        ...
                        <button
                            class="oi-tab"
                            :class="{
                                'oi-tab--active': tabFor(oil.id) === 'sds',
                            }"
                            @click="setTab(oil.id, 'sds')"
                        >
                            SDS Documents
                        </button>
                    </div>
                    ...
                    <!-- ── Hazards ── -->
                    <div
                        v-if="tabFor(oil.id) === 'hazards'"
                        class="oi-tab-body"
                    >
                        <p v-if="!oil.hazards.length" class="oi-empty">
                            No hazards parsed yet. Upload an SDS to
                            auto-populate, or add manually.
                        </p>

                        <div
                            v-else
                            class="adm-card adm-card--flush"
                        ...
                        >
                            <table class="adm-table">
                                ...
                                <tbody>
                                    <template
                                        v-for="hazard in oil.hazards"
                                        :key="hazard.id"
                                    >
                                        <!-- Display row -->
                                        <tr
                                            v-if="editingHazard !== hazard.id"
                                            class="adm-row"
                                        >
                                            <td class="adm-td">
                                                <span class="oi-hcode">{{
                        ...
                        <div class="oi-add-hazard">
                            ...
                            <div v-else class="oi-edit-panel">
                                <div class="oi-edit-grid">
                                    <div class="adm-field">
                                        <label class="adm-label--sm adm-label"
                                            >Code</label
                                        >
                                        <input
                                            v-model="newHazard.hazard_code"
                                            type="text"
                                            class="adm-input adm-input--sm"
                                            placeholder="H317"
                                        />
                                    ...
                                    <div class="adm-field">
                                        ...
                                        >
                                        <input
                                            v-model="newHazard.hazard_class"
                                            type="text"
                                            class="adm-input adm-input--sm"
                                    ...
                                    <div class="oi-edit-actions">
                                        <button
                                            class="adm-btn adm-btn--primary adm-btn--sm"
                                            @click="submitAddHazard(oil.id)"
                                        >
                                            Add
                                        </button>
                                        <button
                                            class="adm-btn adm-btn--ghost adm-btn--sm"
                                            @click="showAddHazard = null"
                                        >
                                            Cancel
                                        </button>
                                    </div>
                    ...
                    <!-- ── Components ── -->
                    <div
                        v-if="tabFor(oil.id) === 'components'"
                        class="oi-tab-body"
                    >
                        <p v-if="!oil.components.length" class="oi-empty">
                            No components parsed yet. Upload an SDS to
                            auto-populate.
                        </p>
                                ...
                                <tbody>
                                    ...
                                    >
                                        ...
                                        >
                                            <span
                                                v-if="
                                                    comp.concentration_min !==
                                                        null &&
                                                    comp.concentration_max !==
                                                        null
                                                "
                                            >
                                                {{ comp.concentration_min }}–{{
                                        ...
                                        <td class="adm-td">
                                            <template
                                                v-if="comp.clp_classification"
                                            >
                                                <span
                                                    v-for="code in comp.clp_classification.split(
                                                        ',',
                                                    )"
                                                    :key="code"
                                                    class="oi-hcode"
                    ...
                    <div v-if="tabFor(oil.id) === 'sds'" class="oi-tab-body">
                        <p v-if="!sdsDocs(oil).length" class="oi-empty">
                            No SDS documents uploaded yet.
                        </p>
                        <div class="oi-sds-list">
                            <div
                                v-for="doc in sdsDocs(oil)"
                                :key="doc.id"
                                class="oi-sds-item"
                            >
                        ...
                        <input
                            :ref="
                                (el) =>
                                    (fileInputRefs[oil.id] =
                                        el as HTMLInputElement)
                            "
                            type="file"
                            accept="application/pdf"
                            style="display: none"
                            @change="handleFileUpload(oil.id, $event)"
                        />

                        <button
                            class="adm-upload-zone"
                            :disabled="uploadingFor === oil.id"
                            @click="triggerUpload(oil.id)"
                        >
                            <svg
                                v-if="uploadingFor !== oil.id"
                                width="18"
                                height="18"
                                viewBox="0 0 24 24"
                                fill="none"
                            ...
                            <span>{{
                                uploadingFor === oil.id
                                    ? 'Uploading…'
                                    : 'Click to upload SDS PDF'
                            }}</span>
                            <span class="adm-upload-note"
                                >Max 10 MB · PDF only</span
                            >
                        </button>
                    </div>
                </div>
...
.oi-tab--active {
    ...
    border-bottom-color: var(--adm-stamp-deep);
    font-weight: 600;
}
