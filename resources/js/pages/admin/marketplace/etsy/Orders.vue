...
const applyFilters = () => {
    router.get(
        route('admin.marketplace.etsy.orders'),
        {
...
const triggerImport = () => {
    ...
    router.post(
        route('admin.marketplace.etsy.orders.import'),
        {},
        {
            onFinish: () => {
...
const statusBadgeClass = (s: string) => {
    switch (s) {
        case 'successful':
            return 'adm-badge adm-badge--on';
        case 'processing':
            return 'adm-badge adm-badge--lav';
        case 'cancelled':
        case 'failed':
        ...
        default:
            return 'adm-badge adm-badge--off';
    }
};
</script>
    ...
    <AdminLayout>
        ...
        <div class="adm-header">
            ...
            <div class="eo-header-actions">
                ...
                <Link
                    :href="route('admin.marketplace.etsy.orders.import-csv')"
                    class="adm-btn adm-btn--ghost adm-btn--sm"
                >
                    Import CSV
                </Link>
        ...
        <div class="eo-filters">
            <input
                v-model="search"
                type="text"
                placeholder="Search email, name or receipt ID…"
                class="adm-input adm-input--sm eo-search-input"
                @keydown.enter="applyFilters"
            />
            <select
                v-model="status"
                @change="applyFilters"
                class="adm-select adm-select--sm eo-status-select"
            >
        ...
        <div class="adm-card adm-card--flush" style="margin-bottom: 1.5rem">
                ...
                <table class="adm-table">
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <td class="adm-td">
                                <div class="eo-customer-name">
                                    {{ o.first_name }} {{ o.last_name }}
                                </div>
                                <div
                                    class="adm-td--mono"
                                    style="font-size: 0.75rem; margin-top: 1px"
                                >
                                    {{ o.email }}
                                </div>
                            </td>
            ...
            <div class="adm-mob-list">
                <div v-for="o in orders.data" :key="o.id" class="adm-mob-card">
                    <div class="eo-mob-head">
                        <div>
                            <div class="eo-order-id">COY-{{ o.id }}</div>
                            <div class="adm-td--mono" style="font-size: 0.7rem">
                                Etsy #{{ o.marketplace_order_id }}
                            </div>
                        </div>
                        <span :class="statusBadgeClass(o.status)">{{
                    ...
                    <div class="eo-mob-meta">
                        <div>
                            ...
                            <p class="adm-td--mono" style="font-size: 0.72rem">
                                {{ o.email }}
                            </p>
                        </div>
                        <div>
            ...
            <div v-if="!orders.data.length" class="adm-empty">
                <div class="adm-empty-icon">
                    <PackageOpen :size="28" :stroke-width="1.5" />
                </div>
                <p class="adm-empty-title">No Etsy orders imported yet</p>
                <p class="adm-empty-sub">
                    Click "Import Now" to pull in your latest Etsy orders.
                </p>
                <button
                    @click="triggerImport"
                    :disabled="importing"
        ...
        <div v-if="orders.last_page > 1" class="adm-pagination">
            <p class="adm-page-info">
                Page <strong>{{ orders.current_page }}</strong> of
                <strong>{{ orders.last_page }}</strong>
            </p>
