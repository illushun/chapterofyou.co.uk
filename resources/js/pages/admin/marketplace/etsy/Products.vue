...
const applyFilters = () => {
    router.get(
        route('admin.marketplace.etsy.products'),
        {
...
const toggleProduct = (product: Product) => {
    ...
    router.post(
        route('admin.marketplace.etsy.products.toggle', product.id),
        {},
        {
            onFinish: () => {
...
const exportProduct = (product: Product) => {
    loadingId.value = product.id;
    router.post(
        route('admin.marketplace.etsy.products.export', product.id),
        {},
        {
            onFinish: () => {
                loadingId.value = null;
...
const syncProduct = (product: Product) => {
    loadingId.value = product.id;
    router.post(
        route('admin.marketplace.etsy.products.sync', product.id),
        {},
        {
            onFinish: () => {
                loadingId.value = null;
...
const unlinkProduct = (product: Product) => {
    if (
        !confirm(
            `Unlink "${product.name}" from Etsy?\n\nThe listing will remain on Etsy but won't be tracked here.`,
        )
    )
        return;
    loadingId.value = product.id;
    ...
    <AdminLayout>
        ...
        <div class="adm-header">
            <div>
                ...
                <p class="adm-sub">
                    <template v-if="connection?.shop_name">
                        Shop:
                        <strong>{{ connection.shop_name }}</strong>
                        &nbsp;·&nbsp;
                    </template>
            ...
            <div class="ep-header-actions">
                <a
                    :href="route('admin.marketplace.etsy.products.export-csv')"
        ...
        <div class="adm-flash adm-flash--warn" style="margin-bottom: 1.25rem">
            ...
            <span>
                <template v-if="connection">
                    Products export as <strong>draft listings</strong>. Add a
                    shipping profile and activate them in your Etsy shop before
                    they go live.
                </template>
                <template v-else>
                    <strong>No API connection yet.</strong>
                    The <strong>Download CSV</strong> button generates a
                    reference sheet of your enabled products. Open it alongside
                    Etsy's listing form to copy titles, descriptions, prices,
                    and tags across without re-typing. Once your API application
                    is approved, you'll be able to push listings directly.
                </template>
            </span>
        </div>
        ...
        <div class="ep-filters">
            <input
                ...
                class="adm-input adm-input--sm ep-search"
                @keydown.enter="applyFilters"
            />
            <select
                v-model="filter"
                @change="applyFilters"
                class="adm-select adm-select--sm ep-filter-select"
            >
        ...
        <div class="adm-card adm-card--flush" style="margin-bottom: 1.5rem">
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <td class="adm-td">
                                ...
                                <div
                                    class="adm-td--mono"
                                    style="font-size: 0.72rem; margin-top: 1px"
                                >
                                    {{ p.mpn }}
                                </div>
                                <!-- Override indicator -->
                                <span
                                    v-if="p.etsy_setting?.has_overrides"
                                    class="adm-badge adm-badge--lav ep-override-badge"
                                >
                                    Overrides set
                                </span>
                            </td>
                            <td class="adm-td adm-td--price">
                                <template v-if="p.etsy_setting?.override_price">
                                    <span class="ep-override-price">{{
                                        fmtCurrency(
                                            p.etsy_setting.override_price,
                                        )
                                    }}</span>
                                    <span class="ep-base-price">{{
                                        fmtCurrency(p.cost)
                                    }}</span>
                                </template>
                                <template v-else>{{
                            ...
                            <td class="adm-td">
                                <span
                                    class="adm-stock"
                                    :class="stockLabel(p.stock_qty).cls"
                                >
                            ...
                            <td class="adm-td">
                                <template v-if="p.etsy_listing">
                                    ...
                                    <div
                                        v-if="p.etsy_listing.sync_error"
                                        class="ep-sync-error"
                                ...
                                <span
                                    v-else-if="isEnabled(p)"
                                    class="adm-badge adm-badge--off"
                                    >Not exported</span
                                >
                                <span v-else class="adm-badge adm-badge--off"
                                    >Disabled</span
                                >
                            </td>
                            <td
                                class="adm-td adm-td--mono"
                                style="font-size: 0.75rem"
                            >
                                {{
                                    formatDate(
                                        p.etsy_listing?.last_synced_at ?? null,
                                    )
                                }}
                            </td>
                            <td class="adm-td adm-td--actions">
                                ...
                                <Link
                                    :href="
                                        route(
                                            'admin.marketplace.etsy.products.settings',
                                            p.id,
                                        )
                                    "
                    ...
                    <div class="ep-mob-meta">
                        ...
                        <div>
                            ...
                            <span
                                class="adm-stock"
                                :class="stockLabel(p.stock_qty).cls"
                                >{{ stockLabel(p.stock_qty).text }}</span
                            >
                        </div>
                        <div>
                            ...
                            <template v-if="p.etsy_listing">
                                <span
                                    :class="
                                        listingBadgeClass(p.etsy_listing.status)
                                    "
                                    >{{
            ...
            <div v-if="!products.data.length" class="adm-empty">
                <div class="adm-empty-icon">
                    <Store :size="28" :stroke-width="1.5" />
                </div>
