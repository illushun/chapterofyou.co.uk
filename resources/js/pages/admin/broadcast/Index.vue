    ...
    <AdminLayout>
        ...
        <div class="adm-header">
            <div>
                ...
                <p class="adm-sub">
                    Send a message to a segment of your customers.
                </p>
            </div>
            <Link
        ...
        <div class="bi-stat-bar">
            ...
            <span
                ><strong>{{ totalOptedIn.toLocaleString() }}</strong> opted-in
                customers in your mailing list</span
            >
        </div>
                ...
                <table class="adm-table">
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <td
                                class="adm-td"
                                style="font-weight: 600; max-width: 240px"
                            >
                                {{ b.subject }}
                            </td>
                            <td class="adm-td">
                            ...
                            <td class="adm-td" style="font-weight: 700">
                            ...
                            <td class="adm-td adm-td--actions">
                                <Link
                                    :href="route('admin.broadcasts.show', b.id)"
            ...
            <div class="adm-mob-list">
                ...
                >
                    ...
                    <p class="bi-mob-meta">
                        ...
                        {{ b.sender?.name ?? '-' }} · {{ fmtDate(b.sent_at) }}
                    </p>
        ...
        <div v-else class="adm-empty" style="margin-bottom: 1.5rem">
            ...
            <p class="adm-empty-sub">
                Compose your first broadcast to get started.
            </p>
            <Link
                :href="route('admin.broadcasts.create')"
                class="adm-btn adm-btn--primary"
        ...
        <div class="bi-waitlist-card">
            <div class="bi-waitlist-head">
                ...
                <div>
                    ...
                    <p class="bi-waitlist-sub">
                        Send the "we're live" email to all waitlist subscribers
                        with their exclusive discount code.
                    </p>
                </div>
            </div>
            ...
            <div class="bi-waitlist-stats">
                <div class="bi-waitlist-stat">
                    <span class="bi-waitlist-val">{{
                        waitlistCount.toLocaleString()
                    }}</span>
                    <span class="bi-waitlist-label">Waitlist subscribers</span>
                </div>
                <div class="bi-waitlist-stat">
                    <span class="bi-waitlist-val bi-waitlist-val--code"
                        >CHAPTERONE</span
                    >
                    <span class="bi-waitlist-label"
                        >Discount code included</span
                    >
                </div>
            </div>
            ...
            <!-- Confirm step -->
            <div
                v-if="waitlistConfirming && !waitlistDone"
                class="bi-waitlist-confirm"
            >
                <p class="bi-waitlist-confirm-text">
                    This will queue the launch email for
                    <strong
                        >{{ waitlistCount.toLocaleString() }} subscriber{{
                            waitlistCount !== 1 ? 's' : ''
                        }}</strong
                ...
                <div class="bi-waitlist-confirm-btns">
                    ...
                    <button
                        @click="waitlistConfirming = false"
                        class="adm-btn adm-btn--ghost adm-btn--sm"
                    >
                        Cancel
                    </button>
                    <button
                        @click="sendWaitlistLaunch"
                        :disabled="waitlistSending"
                        class="bi-waitlist-send-btn"
                    >
                        <svg
                            v-if="waitlistSending"
                            class="adm-spinner"
                            viewBox="0 0 24 24"
                            fill="none"
                        >
                        ...
                        {{
                            waitlistSending
                                ? 'Sending…'
                                : `Yes, send to ${waitlistCount.toLocaleString()}`
                        }}
                    </button>
                </div>
            </div>
            ...
            <button
                v-if="!waitlistConfirming && !waitlistDone"
                @click="waitlistConfirming = true"
                :disabled="waitlistCount === 0"
                class="bi-waitlist-trigger-btn"
            >
                <svg
                    width="14"
                    height="14"
            ...
            <p
                v-if="waitlistCount === 0 && !waitlistDone"
                class="bi-waitlist-empty-note"
            >
...
.bi-stat-bar {
    ...
    background: color-mix(
        in srgb,
        var(--adm-stamp-dim, #c9b8f0) 12%,
        transparent
    );
    border: 1px solid
        color-mix(in srgb, var(--adm-stamp-deep, #9b84d4) 30%, transparent);
    color: var(--adm-ink-dim, #7a7a9a);
    font-size: 0.85rem;
    margin-bottom: 1.25rem;
}

.bi-stat-bar svg {
    color: var(--adm-stamp-deep, #9b84d4);
    flex-shrink: 0;
}

.bi-stat-bar strong {
    color: var(--adm-ink, #1a1a2e);
    font-weight: 700;
}
...
.bi-waitlist-card {
    margin-top: 2rem;
    background: var(--adm-paper-raised, #ffffff);
    border-radius: var(--adm-radius-lg, 14px);
    border: 1px solid var(--adm-stamp-dim, #f2c4ce);
    box-shadow: var(--adm-shadow-card, 0 1px 6px rgba(26, 26, 46, 0.05));
    padding: 1.5rem;
    display: flex;
...
.bi-waitlist-icon {
    ...
    border: 1px solid var(--adm-stamp-dim, #f2c4ce);
    color: var(--adm-stamp-deep, #d4899a);
    display: flex;
    align-items: center;
...
.bi-waitlist-confirm {
    ...
    padding: 1rem;
    border-radius: var(--adm-radius, 8px);
    background: var(--adm-paper, #faf9f7);
    border: 1px solid var(--adm-line, #ece8e2);
}
...
.bi-waitlist-send-btn {
    ...
    border-radius: var(--adm-radius, 8px);
    border: none;
    background: var(--adm-stamp-deep, #d4899a);
...
.bi-waitlist-trigger-btn:hover:not(:disabled) {
    background: var(--adm-stamp-deep, #d4899a);
    border-color: var(--adm-stamp-deep, #d4899a);
    color: var(--adm-paper-raised);
    transform: translateY(-1px);
}
