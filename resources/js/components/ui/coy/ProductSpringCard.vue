...
const cartState=ref<'idle'|'adding'|'added'>('idle');
...
function addToCart(){if(cartState.value!=='idle'||props.product.stock_qty<=0)return;cartState.value='adding';emit('addToCart',props.product.id);cartTimer=setTimeout(()=>{cartState.value='added';cartTimer=setTimeout(()=>{cartState.value='idle'},1800)},550)}
...
@media(prefers-reduced-motion:reduce){.product-visual img,.product-actions button{transition:none}.wishlist--active svg,.product-actions .cart-button--added,.spinner{animation:none}}
</style>
